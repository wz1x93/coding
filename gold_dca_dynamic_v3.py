...
def parse_transactions(transaction_data):
    """
    解析交易数据字符串并计算总成本和总份额
    :param transaction_data: 包含交易数据的字符串
    :return: 总成本, 总份额
    """
    lines_data = []
    # 正则表达式：匹配 交易类型 日期 金额 净值
    transaction_pattern = re.compile(r'^(定投|买入|卖出)\s+(\d{4}-\d{2}-\d{2})\s+([\d,]+\.\d{2})\s+([-]?\d+\.\d+)')
    ...
    for line in transaction_data.strip().split('\n'):
        ...
        if match:
            transaction_type, date_str, amount_str, nav_str = match.groups()
            ...
            try:
                ...
                lines_data.append((date_obj, transaction_type, amount, nav))
    ...
    for _, transaction_type, amount, nav in lines_data:
        if transaction_type in ['定投', '买入']:
            shares_add = amount / nav
            total_cost += amount
            total_shares += shares_add
            
        elif transaction_type == '卖出' and total_shares > 0:
...
def calculate_config():
    ...
    # 2. 计算 14 天前的时间
    start_date_obj = current_time - timedelta(days=14)
...
def calculate_normalized_atr(stock_code, start_date, end_date, atr_period=14):
    """
    计算指定股票的归一化ATR
    :param stock_code: 股票代码，例如 '000216'
    :param start_date: 开始日期 'YYYY-MM-DD'
    :param end_date: 结束日期 'YYYY-MM-DD'
    :param atr_period: ATR周期，默认为14
    ...
    k_data_result = bs.query_history_k_data_plus(stock_code, fields, start_date, end_date, frequency="d", adjustflag="3")

    if k_data_result.error_code != '0':
        print(f"获取数据失败: {k_data_result.error_msg}")
        bs.logout()
        return None

    data_list = []
    while (k_data_result.error_code == '0') & k_data_result.next():
        data_list.append(k_data_result.get_row_data())
    
    df = pd.DataFrame(data_list, columns=k_data_result.fields)
    ...
    tr1 = df['high'] - df['low']
    tr2 = abs(df['high'] - df['previous_close'])
    tr3 = abs(df['low'] - df['previous_close'])
    df['tr'] = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1)

    # 5. 计算 ATR (Average True Range)
    # 这里使用 Wilder 平滑方法计算ATR
    df['atr'] = np.nan
...
class GoldDCAStrategy:
    def __init__(self, base_daily=100.0, max_daily=5000.0):
        """
        初始化策略对象
        :param base_daily: 基础每日投资金额
        :param max_daily: 最大每日投资金额
        """
    ...
    def monte_carlo_breakeven_prob(self, current_nav, days=90, n_sims=1000):
        """
        使用蒙特卡洛模拟计算回本概率
        :param current_nav: 当前净值
        :param days: 模拟天数
        :param n_sims: 模拟次数
        :return: 回本概率和预计中位回本天数
        """
    ...
    def daily_decision(self, current_nav):
        ...
        print("-" * 50)
        print(f"当前NAV: {current_nav:.4f}")
        print(f"平均成本: {avg_cost:.4f}")
        print("-" * 30)
        print(f"当前持仓市值: {current_value:.2f} 元")
        print(f"累计盈亏: {pnl_abs:+.2f} 元 ({pnl_pct:+.2f}%)")
        print("-" * 30)
        print(f"MC 90天回本概率: {prob:.1f}%")
        print(f"预计中位回本天数: {median_days:.0f}天")
        print("-" * 30)
        print(f"操作信号: {signal}")
        print("-" * 30)
        print(f"执行后新平均成本: {self.get_current_avg_cost():.4f}")
        print(f"新总份额: {self.total_shares:.2f}")
        print(f"新总成本: {self.total_cost:.2f}")
        print("-" * 50)
